(function () {
  function e() {
    if (document.getElementById(`tlc-sync-container`)) return !0; let e = document.querySelector(`.panel.panel-theme`); if (!e || !e.parentNode) return !1; let n = document.createElement(`style`); n.textContent = `
    #tlc-sync-container {
      margin: 0 0 18px 0;
      display: flex;
  ...
  `, document.body.appendChild(n), document.getElementById(`tlc-btn-cancel`).onclick = () => { n.remove(), c(t) }, document.getElementById(`tlc-btn-confirm`).onclick = () => { let e = document.getElementById(`tlc-session-select`), r = e.value, i = e.options[e.selectedIndex]?.text || `Session`; n.remove(), o(r, i, t) }
  } function i() { let e = document.querySelector(`input[id$="-attended"]`); if (!e) return null; let t = e.id.split(`-`); return t.length >= 3 ? t[1] : null } function a(e, t) { let n = `${e}-${t}-attended`, r = document.getElementById(n); if (!r) return `notfound`; if (r.value === `1`) return `skipped`; let i = r.closest(`.cbx-container`)?.querySelector(`.cbx`); return i ? (i.click(), `checked`) : `notfound` } function o(e, t, n) {
    n.innerHTML = `⏳ Syncing...`, chrome.runtime.sendMessage({ action: `SYNC_ATTENDANCE`, sessionId: e }, r => {
      if (chrome.runtime.lastError || r?.error) { alert(`Sync failed: ` + (chrome.runtime.lastError?.message || r?.error)), c(n); return } let o = r.data || []; if (console.log(`[TLC Sync] Scans received from Supabase:`, o), o.length === 0) { s(t, 0, 0, [], [], n), chrome.runtime.sendMessage({ action: `MARK_SESSION_SYNCED`, sessionId: e }); return } let l = i(); if (console.log(`[TLC Sync] TLC Event ID extracted from page:`, l), !l) {
...
Make sure you have selected an event on the TLC Track Attendance page so the roster has loaded.`), c(n); return
    ...
    let o = document.createElement(`div`); o.id = `tlc-modal-overlay`, o.innerHTML = `
    <div id="tlc-modal">
      <h2>Sync Complete!</h2>
  ...
  `, document.body.appendChild(o), document.getElementById(`tlc-results-ok`).onclick = () => { o.remove(), c(a) }
