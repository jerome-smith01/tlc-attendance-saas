...
- **Billing Admin (`billing_admin`)**: The primary account owner for a troop. Has full control over the troop's subscription, billing, and roster, including managing other leaders. (Limit: One per troop in current scope).
- **Troop Admin (`troop_admin`)**: A leader with full operational control over the troop's roster, events, and attendance. Cannot manage billing.
...
1. **Billing Admin**: Creates the troop account, sets up the subscription, and receives the `billing_admin` role automatically.
2. **Troop Admin & Badge Scanner**: Invited by a Billing Admin or another Troop Admin via the "Invite Leader" flow. Receives an email invitation.
...
A planned feature where parents/members can scan a QR code at meetings to request to join a troop, requiring approval from a Troop Admin or Billing Admin.
